...
function widget($args, $instance) {
  ...
  if ( in_array($instance['sort_by'], $valid_sort_orders) ) {
	...
	if (($instance['featured_prod']) && ($instance['onsale_prod'])) {
		$args = array(
  ...
  $cat_posts = new WP_Query($args);
		...
		<li class="cat_post_item-<?php echo $posts_counter; ?> clearfix">
            ...
            <div class="post-meta-wrapper">	
				<?php if ( $instance['date'] ) : ?>
                    <span class="post_meta"><time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('F jS, Y'); ?></time></span>
                <?php endif; ?>    
            ...
            <?php if ( $instance['show_title'] ) : ?>
			 <?php if ( $instance['before_post_title'] ) { ?>
             ...
             <?php } else { ?>
             	<h5><a class="post-title" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php if ( $instance['show_title_date'] ) {?><?php the_time('M d, Y'); ?><?php }else{?><?php the_title(); ?><?php }?></a></h5>
             <?php } ?>
			<?php endif; ?>
            ...
            <div class="shop-options">
                ...
                <?php }?>
                <?php if ($instance['show_price']) { ?>
                	<div class="prod_price">
						<?php 
                            $_product = new jigoshop_product( get_the_ID() );
                            do_action('jigoshop_after_shop_loop_item_title', $cat_posts, $_product); 
                        ?>
                    </div>
                <?php }?>
            </div>
	...
	<?php if ( $instance['global_link'] ) : ?>
	  <a href="<?php echo $instance['global_link_href']; ?>" <?php if ( $instance['global_link_class'] !='' ) { ?>class="<?php echo $instance['global_link_class']; ?>"<?php } else { ?>class="button"<?php } ?>><?php if($instance['global_link_text']==""){ ?>View all<?php }else{ ?><?php echo $instance['global_link_text']; ?><?php } ?></a>
	<?php endif; ?>
...
<fieldset style="border:1px solid #F1F1F1; padding:10px 10px 0; margin-bottom:1em;">
    <legend style="padding:0 5px;"><?php _e('Shop options'); ?>:</legend>
    <p>
      <label for="<?php echo $this->get_field_id("featured_prod"); ?>">
          <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("featured_prod"); ?>" name="<?php echo $this->get_field_name("featured_prod"); ?>"<?php checked( (bool) $instance["featured_prod"], true ); ?> />
          <?php _e( 'Select only featured products' ); ?>
      </label>
    </p>
    <p>
      <label for="<?php echo $this->get_field_id("onsale_prod"); ?>">
          <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("onsale_prod"); ?>" name="<?php echo $this->get_field_name("onsale_prod"); ?>"<?php checked( (bool) $instance["onsale_prod"], true ); ?> />
          <?php _e( 'Select only products on sale' ); ?>
      </label>
    </p>
    <br />
    <p>
		<label for="<?php echo $this->get_field_id("show_price"); ?>">
          <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("show_price"); ?>" name="<?php echo $this->get_field_name("show_price"); ?>"<?php checked( (bool) $instance["show_price"], true ); ?> />
          <?php _e( 'Show product price' ); ?>
      	</label>
    </p>
    <p>
...
<div style="width:230px; float:left; padding-left:20px;">
  ...
  <p>
      <label for="<?php echo $this->get_field_id("global_link_class"); ?>">
          <?php _e( 'Link class' ); ?>:
          <input class="widefat" id="<?php echo $this->get_field_id("global_link_class"); ?>" name="<?php echo $this->get_field_name("global_link_class"); ?>" type="text" value="<?php echo esc_attr($instance["global_link_class"]); ?>" /><span style="font-size:11px; color:#999;"><?php _e( '(default: "button")' ); ?></span>
      </label>
  </p>
  </fieldset>
</div>
