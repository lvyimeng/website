				...
				if ($thumb == 'true') {
						if ( has_post_thumbnail($post->ID) ){
							...
							$output .= '<h6><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
									$output .= my_string_limit_words($post_title,$title_excerpt_count);
							$output .= '</a></h6>';
							if($meta == 'true'){
									$output .= '<span class="meta">';
											...
											$output .= '<span class="post-comments">';
												$output .= '<a href="'.get_comments_link($post->ID).'">';
													$output .= get_comments_number($post->ID).' Comment(s)';
												$output .= '</a>';
											$output .= '</span>';
									$output .= '</span>';
							}
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme2196");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme2196");
