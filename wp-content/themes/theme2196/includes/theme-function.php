...
function my_string_limit_char($excerpt, $substr=0)
{
	...
	if (strlen($excerpt) > strlen ($string)) {
		$string .= '...';
	}
	return $string;
		}
...
function theme2196_random($length){
...
/*-----------------------------------------------------------------------------------*/
/*    Custom Links for Top Menu
/*-----------------------------------------------------------------------------------*/
function login_out_function ($nav, $args){
	
  if( 'shop_menu' === $args -> theme_location ) {
	if ( of_get_option('g_add_links_id') == 'yes') {
		if (!is_user_logged_in()) {
			$logtext = __('Login');
			$logurl = wp_login_url(home_url());
		} else {
			$logtext = __('Logout');
			$logurl = wp_logout_url(home_url());
		}
		if (!is_user_logged_in()) {
		$regtext = __('Register');
		$regurl = site_url('/wp-login.php?action=register&redirect_to=' . get_permalink());
		} else {
			$regtext = __('Site Admin');
			$regurl = site_url('/wp-admin');
		}
		return '<li><a href="'.$regurl.'">'.$regtext.'</a></li><li><a href="'.$logurl.'">'.$logtext.'</a></li>'.$nav;
	} else {return $nav;}
  } else {
	  return $nav;
  }
...
{ 
     ...
     if(1 != $pages)
     {
         ...
         if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."' class='pagenavi_prev'>First</a>";
         if($paged > 1) echo "<a href='".get_pagenum_link($paged - 1)."' class='pagenavi_prev'>Prev</a>";
         ...
         if ($paged < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\" class='pagenavi_next'>Next</a>"; 
         if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."' class='pagenavi_next'>Last</a>";
         echo "</div>\n";
     }
}
