...
class MY_RecentProducts extends WP_Widget {
    ...
    function widget($args, $instance) {	
		...
		$exc_count = apply_filters('exc_count', $instance['exc_count']);
		$query_args = array(
							...
							<div class="widgetHolder">
                                ...
                                <?php while( $prod->have_posts() ) : $prod->the_post(); $_product = new jigoshop_product( get_the_ID() ); $loop++;?>
                                    <li class="product <?php if ($loop%$columns==0) echo 'last'; if (($loop-1)%$columns==0) echo 'first'; ?>">
                                        ...
                                        <div class="caption">
                                            <h3 class="title"><a href="<?php the_permalink(); ?>" class="product-info">
												<?php $title = the_title('','',FALSE); 
													  if (($exc_count!='0') or ($exc_count!='')) {
														echo my_string_limit_char($title, $exc_count);
                                                      } else {
                                                      	the_title();
                                                      }
												?>
    ...
    function form($instance) {				
        $title = esc_attr($instance['title']);
				...
				$exc_count = esc_attr($instance['exc_count']);
        ?>
       <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'theme2196'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
             ...
             <p>
             <p><label for="<?php echo $this->get_field_id('exc_count'); ?>"><?php _e('Product name excerpt length(chars):', 'theme1835'); ?> <input id="<?php echo $this->get_field_id('exc_count'); ?>" size="2" name="<?php echo $this->get_field_name('exc_count'); ?>" type="text" value="<?php echo $exc_count; ?>" /></label></p>
