...
if (have_posts()) : while (have_posts()) : the_post(); $_product = &new jigoshop_product( $post->ID ); $loop++;
	...
	<li class="product <?php if ($loop%$columns==0) echo 'last'; if (($loop-1)%$columns==0) echo 'first'; ?>">
		...
		<div class="caption">
            ...
            <div class="short-desc"><?php echo my_string_limit_words($excerpt,7).'...'; ?></div>
			<?php jigoshop_template_loop_add_to_cart( $post, $_product ) ?>
		</div>
        </div>
	</li><?php 
...
if ($loop==0) :
	...
	echo '<p class="info">'.__('No products found which match your selection.', 'theme2196').'</p>'; 
	...
	function jigoshop_template_loop_add_to_cart( $post, $_product ) {
		...
		if ( $_product->is_in_stock() OR $_product->is_type('external') ) :
			if ( $_product->is_type(array('variable', 'grouped')) ) :
				$output = '<a href="'.get_permalink($_product->id).'" class="button">'.__('Selsdfsdfect', 'theme2196').'</a>';
			elseif ( $_product->is_type('external') ) :
				$output = '<a href="'.get_post_meta( $_product->id, 'external_url', true ).'" class="button">'.__('Buy product', 'theme2196').'</a>';
			else :
				$output = '<a href="'.esc_url($_product->add_to_cart_url()).'" class="button">'.__('Add to cart', 'theme2196').'</a>';
			endif;
		elseif ( ($_product->is_type(array('grouped')) ) ) :
			return;
		else :
			$output = '<span class="nostock">'.__('Out of Stock', 'theme2196').'</span>';
		endif;
		echo $output;
